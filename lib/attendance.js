...
function attendance () {
  this.data = {
    ...
    responsedPersonList: attendData.responsedPersonList
  }
}
...
attendance.prototype.getMessage = function (isReset) {
  if (!this.isNotyet() || isReset){
    var attendListArray = []
    var absentListArray = []
    for (var index in this.data.attend) {
      attendListArray.push(this.data.responsedPersonList[this.data.attend[index]])
    }
    for (var index in this.data.absent) {
      absentListArray.push(this.data.responsedPersonList[this.data.absent[index]])
    }
...
attendance.prototype.addAttend = function (chatID, name) {
  ...
  this.addResponsedPersonList(chatID, name)
  this.saveToFile()
}
...
attendance.prototype.addResponsedPersonList = function (chatID, name) {
  this.data.responsedPersonList[chatID] = name
}

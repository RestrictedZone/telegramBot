...
attendance.prototype.addAttend = function (name) {
  if(this.data.attend.indexOf(name) === -1){
    console.log('addAttend: push name')
    this.data.attend.push(name)
  }
  if(this.data.absent.indexOf(name) !== -1){
    console.log('addAttend: pop name')
    this.data.absent.splice(this.data.absent.indexOf(name), 1)
  }
  console.log('addAttend:', name, this.data.attend.indexOf(name), this.data.absent.indexOf(name)/*, this.data */)
  this.saveToFile()
}

attendance.prototype.addAbsent = function (name) {
  if(this.data.absent.indexOf(name) === -1){
    console.log('addAbsent: push name')
    this.data.absent.push(name)
  }
  if(this.data.attend.indexOf(name) !== -1){
    console.log('addAbsent: pop name')
    this.data.attend.splice(this.data.attend.indexOf(name), 1)
  }
  console.log('addAbsent:', name, this.data.attend.indexOf(name), this.data.absent.indexOf(name)/*, this.data*/ )
  this.saveToFile()
}

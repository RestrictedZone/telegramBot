...
module.exports.processForOCR = function(originalFilePath, destinationFilePath, callback, chatId, language) {
  // console.log("cropImage execute!");
  if(fs.existsSync(originalFilePath)){
    gm(originalFilePath).size(function(err, value){
      var width = value.width
      var height = value.height
      gm(originalFilePath)
      .resize(width * 2, height * 2) // size up twice
      .channel('Gray') // change image to 'Grayscale'
      .threshold(80, true)
      .crop(width * 2, height, 0, height + 420)
      .write(destinationFilePath, function (err) {
        fs.chmodSync(originalFilePath, 777);
        if (!err) {
          // console.log(destinationFilePath + ' has arrived');
